...
class Bingo {
 public:
  ...
  friend bool operator==(const Bingo first, const Bingo second);
...
bool operator==(const Bingo first, const Bingo second);
  ...
  for (const auto number : numbers) {
    for (auto& board : boards) {
      ...
      if (board.check_for_winner()) {
        if ((boards.size() == 1) && (boards[0].check_for_winner())) {
          std::cout << "Losser:" << std::endl;
          boards[0].print_board();
          const int score = number * boards[0].get_sum_of_winner();
          std::cout << "Lossing Score: " << score << std::endl;
          return 0;
        }
      }
    }
    boards.erase(
        std::remove_if(boards.begin(), boards.end(),
                       [](Bingo board) { return board.check_for_winner(); }),
        boards.end());
...
bool operator==(const Bingo first, const Bingo second) {
  for (int row = 0; row < GRID_SIZE; row++) {
    for (int col = 0; col < GRID_SIZE; col++) {
      if (first.full_board[row][col] != second.full_board[row][col]) {
        return false;
      }
      if (first.in_progress_board[row][col] !=
          second.in_progress_board[row][col]) {
        return false;
      }
    }
  }
